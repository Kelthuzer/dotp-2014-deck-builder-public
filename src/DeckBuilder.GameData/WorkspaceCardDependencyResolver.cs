...
internal static class WorkspaceCardDependencyResolver
{
        ...
        string currentReference)
    {
        ...
        IReadOnlyList<string> cwTokenKeys = xml.Contains("CW_Tokens", StringComparison.OrdinalIgnoreCase)
            ? WorkspaceRuntimeCompatibility.ExtractCwTokenKeys(xml)
            : Array.Empty<string>();

        // TOKEN_REGISTRATION is required even for tokens whose definition is generated by CW_Tokens.
        // If a concrete CARD_V2 exists, package it. If it does not and the registration exactly
        // matches a CW_Tokens archetype used by this card, runtime compatibility owns that token.
        foreach (string registeredToken in ExtractRegisteredTokenReferences(xml))
        {
            ...
            if (TryResolveReferenceAlias(registeredToken, referenceAliases, out string canonicalReference))
            {
                references.Add(canonicalReference);
                     ...
                     && !WorkspaceRuntimeCompatibility.IsDynamicCwRegistration(registeredToken, cwTokenKeys))
            {
