...
namespace RSN.DotP
{
	public class CardInfo
	{
		...
		public static void ClearSharedImageCache()
		{
			if (m_dicCastingCostCache == null)
				return;

			foreach (Bitmap image in m_dicCastingCostCache.Values.Distinct())
				image.Dispose();
        ...
        public void Dispose()
        {
			// Casting cost images are shared by every CardInfo through the static cache.
			// They are released once by GameDirectory.Dispose, never by an individual card.
			m_imgCastingCost = null;
            m_gdData = null;
