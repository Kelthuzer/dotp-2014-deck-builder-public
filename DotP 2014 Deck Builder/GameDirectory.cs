...
namespace RSN.DotP
{
	public class GameDirectory
	{
		...
		private Dictionary<string, CardInfo> m_dicCardsByFilename;
		private SortableBindingList<Deck> m_lstDecks;
		private Dictionary<KeyValuePair<string, LoadImageType>, WadBase> m_dicImages;
		private Dictionary<string, KeyValuePair<string, LoadImageType>> m_dicImagesByName;
		...
		{
			CardInfo ciCard;
			return strFileName != null && m_dicCardsByFilename != null && m_dicCardsByFilename.TryGetValue(strFileName, out ciCard)
				? ciCard
				: null;
		}
		...
		public TdxWrapper LoadImage(string strImage, LoadImageType eType = LoadImageType.SearchAll)
		{
			if (strImage != null)
			{
				...
				else if (m_dicImagesByName != null)
					m_dicImagesByName.TryGetValue(strImage, out kvpFound);

				if (!String.IsNullOrEmpty(kvpFound.Key))
				{
		...
		public bool HasImage(string strImage, LoadImageType eType = LoadImageType.SearchAll)
		{
			if (String.IsNullOrEmpty(strImage) || m_dicImages == null)
				return false;

			strImage = strImage.ToUpperInvariant();
			if (eType != LoadImageType.SearchAll)
				return m_dicImages.ContainsKey(new KeyValuePair<string, LoadImageType>(strImage, eType));
			return m_dicImagesByName != null && m_dicImagesByName.ContainsKey(strImage);
		}
		...
		public void LoadWads()
		{
			...
			m_dicCardsByFilename = new Dictionary<string, CardInfo>(StringComparer.OrdinalIgnoreCase);
			m_dicImagesByName = new Dictionary<string, KeyValuePair<string, LoadImageType>>(StringComparer.OrdinalIgnoreCase);
			...
			if (Directory.Exists(m_strGameDirectory))
			{
				...
				// Build lookup tables once. Deck and image loading below calls these paths thousands of times.
				foreach (CardInfo card in m_lstCards)
				{
					if (card.Filename != null && !m_dicCardsByFilename.ContainsKey(card.Filename))
						m_dicCardsByFilename.Add(card.Filename, card);
				}
				foreach (KeyValuePair<string, LoadImageType> imageKey in m_dicImages.Keys)
				{
					if (!m_dicImagesByName.ContainsKey(imageKey.Key))
						m_dicImagesByName.Add(imageKey.Key, imageKey);
				}
        ...
        public void Dispose()
        {
			...
			CardInfo.ClearSharedImageCache();
			if (m_dicCardsByFilename != null)
			{
				m_dicCardsByFilename.Clear();
				m_dicCardsByFilename = null;
			}
			if (m_dicImagesByName != null)
			{
				m_dicImagesByName.Clear();
				m_dicImagesByName = null;
			}
